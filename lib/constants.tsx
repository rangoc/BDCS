import anastasiiai from "../public/people/AnastasiiaIshkinina.webp";
import andjelinalar from "../public/people/AndjelinaRakcevic.webp";
import anjaj from "../public/people/AnjaJovanovic.webp";
import bobanc from "../public/people/BobanCabarkapa.webp";
import bojanab from "../public/people/BojanaBoskovic.webp";
import bojanav from "../public/people/BojanaVucicevic.webp";
import bojanz from "../public/people/BojanZivkovic.webp";
import dijanao from "../public/people/DijanaOpancina.webp";
import drazenm from "../public/people/DrazenMilic.webp";
import dritand from "../public/people/DritanDragovic.webp";
import eldarb from "../public/people/EldarBerkovac.webp";
import eminaa from "../public/people/EminaAslama.webp";
import eminac from "../public/people/EminaCobovic.webp";
import eminak from "../public/people/EminaKonak.webp";
import hasijai from "../public/people/HasijaIsakovic.webp";
import ivam from "../public/people/IvaMilacic.webp";
import ivanaj from "../public/people/IvanaJovanovic.webp";
import ivanaz from "../public/people/IvanaZvicer.webp";
import ivanv from "../public/people/IvanVuskovic.webp";
import jelenas from "../public/people/JelenaSimic.webp";
import jovanaj from "../public/people/JovanaJanketic.webp";
import jovanap from "../public/people/JovanaPopovic.webp";
import kristinar from "../public/people/KristinaRadovic.webp";
import matijav from "../public/people/MatijaVujovic.webp";
import merisaa from "../public/people/MerisaAganbegovic.webp";
import milenar from "../public/people/MilenaRaicevic.webp";
import milicaj from "../public/people/MilicaJaric.webp";
import milkaj from "../public/people/MilkaJovanovic.webp";
import milosv from "../public/people/MilosVuletic.webp";
import nemanjak from "../public/people/NemanjaKljajevic.webp";
import nenadv from "../public/people/NenadVucinic.webp";
import nikolam from "../public/people/NikolaMaric.webp";
import nikolar from "../public/people/NikolaRadunovic.webp";
import nikolinaj from "../public/people/NikolinaJankovic.webp";
import nikolinak from "../public/people/NikolinaKoljevic.webp";
import petarm from "../public/people/PetarMilas.webp";
import sabrinal from "../public/people/SabrinaLukovic.webp";
import stefanp from "../public/people/StefanPavlovic.webp";
import zerinas from "../public/people/ZerinaSikira.webp";

const BREAKPOINTS = {
  mobileMax: 550,
  tabletMax: 1100,
  laptopMax: 1500,
};

export const QUERIES = {
  mobileAndDown: `(max-width: ${BREAKPOINTS.mobileMax}px)`,
  tabletAndDown: `(max-width: ${BREAKPOINTS.tabletMax}px)`,
  laptopAndDown: `(max-width: ${BREAKPOINTS.laptopMax}px)`,
};

export const WHY_CHOOSE_US = [
  {
    title: "Professional Approach",
    description: `By working together creatively from the start of each engagement, we're able to combine our shared knowledge to devise
sustainable solutions for our clients. From launch to
completion, the same core team will work on your project and
make sure that your demands are always met. We stand by our
quality; it's all we know and all we practice.`,
    image: (
      <svg
        preserveAspectRatio="xMidYMid meet"
        data-bbox="0 16.7 200.1 166.7"
        xmlns="http://www.w3.org/2000/svg"
        width="200"
        height="200"
        viewBox="0 16.7 200.1 166.7"
        role="presentation"
        aria-hidden="true"
      >
        <g>
          <path d="M195.8 41.7h-52.1c0-.1.1-.2.1-.4 0-13.8-11-24.6-25-24.6H85.4c-14 0-25 10.6-25 24.2 0 .3.1.5.2.8H4.2c-2.3 0-4.2 1.9-4.2 4.2v133.3c0 2.3 1.9 4.2 4.2 4.2h191.7c2.3 0 4.2-1.9 4.2-4.2V45.8c-.1-2.3-2-4.1-4.3-4.1zm-127-.8C68.8 32 76.1 25 85.4 25h33.3c9.3 0 16.7 7.2 16.7 16.3 0 .1.1.2.1.4H68.6c.1-.3.2-.5.2-.8zM191.7 175H8.3v-71.2c6.6 7.7 16.3 12.9 26.6 12.9h134.4c8.9 0 16.8-4.1 22.4-10.6V175zm0-92.7c0 12.8-8.4 26-22.4 26H34.9c-13.7 0-26.6-12.6-26.6-26V50h183.3l.1 32.3zm-89.6 20.4c6.9 0 12.5-5.6 12.5-12.5s-5.6-12.5-12.5-12.5-12.5 5.6-12.5 12.5c0 6.8 5.6 12.5 12.5 12.5zm0-16.7c2.3 0 4.2 1.9 4.2 4.2 0 2.3-1.9 4.2-4.2 4.2s-4.2-1.9-4.2-4.2 1.9-4.2 4.2-4.2z"></path>
        </g>
      </svg>
    ),
  },
  {
    title: "Competitive Pricing",
    description: `Our experienced team in audit offers competitive pricing for our support, ensuring that you get the highest quality working papers for a reasonable fee. Our experienced team will be involved from the initiation to the finalization of the engagement, making sure that every step of the process is handled with the utmost care and attention.`,
    image: (
      <svg
        preserveAspectRatio="xMidYMid meet"
        data-bbox="7.961 0.012 184.079 200.088"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="7.961 0.012 184.079 200.088"
        role="presentation"
        aria-hidden="true"
      >
        <g>
          <path d="M191.8 177.8C185.5 152 144.9 72 143.2 68.6l-1.2-2.3H58l-1.2 2.3C55.1 72 14.5 152.4 8.2 178.2c-.4 1.8-.2 4.4-.1 5.5.2 8.8 10.4 16.4 18.8 16.4h146.2c8.9 0 18.8-6.7 18.8-15.3.1-1.9.3-5.2-.1-7zm-8.3 6.6c0 3.7-6 7.3-10.4 7.3H26.9c-4.7 0-10.4-4.6-10.4-8.3v-.5c-.1-.9-.2-2.3-.1-2.8 5.5-22.5 40-92 46.8-105.5H137c6.8 13.5 41.3 82.6 46.7 105 0 .6 0 2.7-.2 4.8zM140.8 49.6c2.3 0 4.2 1.9 4.2 4.2 0 2.3-1.9 4.2-4.2 4.2H57.2c-2.3 0-4.2-1.9-4.2-4.2 0-2.3 1.9-4.2 4.2-4.2h83.6zm-66.3-8.4h17.1c1.3 0 2.4-.6 3.2-1.5.8-1 1.1-2.2.9-3.5C91.5 14.9 79.4 1.4 63.4.1 51.9-.8 41.1 5.3 37.6 14.7c-1.5 4.1-1.2 8.1 1 11.6 7.1 11.5 33 14.6 35.9 14.9zM45.4 17.6c2.2-5.9 9.5-9.8 17.3-9.2 10.9.9 19.5 9.9 23.7 24.4H75.1c-9.5-1-25.7-4.9-29.5-11-.7-1.2-.8-2.6-.2-4.2zm62.3 23.6h17.1c2.9-.3 28.8-3.3 35.9-14.9 2.2-3.5 2.5-7.6 1-11.6C158.2 5.3 147.4-.8 136 .1c-16 1.3-28.1 14.8-32.4 36.1-.2 1.2.1 2.5.9 3.5.8.9 1.9 1.5 3.2 1.5zm28.9-32.8c7.8-.6 15 3.2 17.3 9.2.6 1.6.5 3-.3 4.3-3.7 6.1-20 9.9-29.5 11H113c4.1-14.6 12.8-23.6 23.6-24.5zm-14.2 141.1c0 10.2-7.1 17.3-17.9 18.8v5.1c0 2.3-1.9 4.2-4.2 4.2s-4.2-1.9-4.2-4.2v-5.1c-10.9-1.5-17.9-8.6-17.9-18.8 0-2.3 1.9-4.2 4.2-4.2 2.3 0 4.2 1.9 4.2 4.2 0 10 10.5 10.7 13.7 10.7s13.7-.8 13.7-10.7c0-6.2-2.4-7.7-12.7-12.7l-5.8-2.9c-10.1-5-17.4-8.6-17.4-20.2 0-10.2 7.1-17.5 17.9-18.9v-5c0-2.3 1.9-4.2 4.2-4.2s4.2 1.9 4.2 4.2v5c10.7 1.5 17.7 8.7 17.7 18.9 0 2.3-1.9 4.2-4.2 4.2-2.3 0-4.2-1.9-4.2-4.2 0-10.1-10.3-10.9-13.5-10.9-6.6 0-13.7 2.9-13.7 10.9 0 6.2 2.4 7.6 12.7 12.7l5.8 2.9c10.2 5 17.4 8.6 17.4 20.2z"></path>
        </g>
      </svg>
    ),
  },
  {
    title: "Personalized attention",
    description: `Every year, our work experience allows us to become better at serving clients. With every engagement we take on, we're better able to understand their needs, focusing our attention and resources on what's truly important. Work with a solid and established firm that won't let you down. Work with BD Corporate Services.`,
    image: (
      <svg
        preserveAspectRatio="xMidYMid meet"
        data-bbox="0 0.744 200 198.556"
        xmlns="http://www.w3.org/2000/svg"
        width="200"
        height="200"
        viewBox="0 0.744 200 198.556"
        role="presentation"
        aria-hidden="true"
      >
        <g>
          <path d="M144.8 80.7V63.4c0-4 0-10.9-8.7-12.6V37.5c0-14.8-4.6-25.3-13.7-31.3-17.9-11.8-46.7-1.1-52.1 1C54.1 7.7 41 21.1 41 37.5v14.7c-3.4 2-8.7 5.9-8.7 11.3v17.3c0 6.2 6.3 10.8 12.1 12.4 1.3 15.9 10.6 33.2 24 43.4l-10.2 15.7H47.6c-28 0-47.6 17.3-47.6 42.1 0 2.4 1.9 4.3 4.3 4.3s4.3-1.9 4.3-4.3c0-23.1 19.6-33.5 38.9-33.5h13c1.5 0 2.8-.7 3.6-2l11.5-17.7c5.5 2.8 11.5 4.5 17.8 4.5 25.4 0 43.5-27.1 45-52.7 3.2-1.4 6.4-4.6 6.4-12.3zm-8.7 0c0 4.3-1.1 4.3-1.8 4.3-2.4 0-4.3 1.9-4.3 4.3 0 22.5-15.6 47.6-36.4 47.6-21.9 0-40.6-26.2-40.6-47.6 0-2.4-1.9-4.3-4.3-4.3-3.4 0-7.6-2.9-7.6-4.3V63.4c.2-1 3-3.4 6.1-4.7 1.6-.7 2.6-2.2 2.6-3.9V37.5c0-11.9 9.7-21.6 21.6-21.6.6 0 1.2-.1 1.7-.3.2-.1 14.7-6.2 28.6-6.2 5.8 0 11.5 1.1 16.1 4.1 6.5 4.3 9.8 12.4 9.8 24.1v17.3c0 2.4 1.9 4.3 4.3 4.3 4.3 0 4.3 0 4.3 4.3v17.2zm42.4 49.3c-6 0-11.6 2.5-15.6 6.7-4-4.3-9.7-6.8-15.7-6.8-11.9 0-21.5 9.8-21.5 21.8 0 5.3 1.9 10.6 5.2 14.3.1.1.2.2.2.3l28.6 31.6c.8.9 2 1.4 3.2 1.4 1.2 0 2.4-.5 3.2-1.4l28.5-31.6c.1-.1.1-.2.2-.3 3.3-3.7 5.2-9 5.2-14.4 0-11.9-9.7-21.6-21.5-21.6zm9.7 30.4c-.1.1-.2.2-.3.4l-25 27.8-25.1-27.7c-.1-.1-.2-.3-.3-.4-1.9-2-3.2-5.5-3.2-8.7 0-7.1 5.9-13.2 12.9-13.2 5 0 9.6 3 11.7 7.6.7 1.5 2.2 2.5 3.9 2.5 1.7 0 3.2-1 3.9-2.5 2.1-4.5 6.7-7.5 11.7-7.5 7.1 0 12.9 5.8 12.9 13 0 3.2-1.2 6.7-3.1 8.7z"></path>
        </g>
      </svg>
    ),
  },
  {
    title: "National Standards",
    description: `Our teams are led by experienced Big 4 professionals who are ACCA qualified and IFRS experts. Our team leader also has extensive Dutch GAAP experience. We are dedicated to providing our clients with the highest quality of services.`,
    image: (
      <svg
        preserveAspectRatio="xMidYMid meet"
        data-bbox="16.6 0 166.7 200.1"
        xmlns="http://www.w3.org/2000/svg"
        width="200"
        height="200"
        viewBox="16.6 0 166.7 200.1"
        role="presentation"
        aria-hidden="true"
      >
        <g>
          <path d="M179.2 0H20.8c-2.3 0-4.2 1.9-4.2 4.2v191.7c0 2.3 1.9 4.2 4.2 4.2h158.3c2.3 0 4.2-1.9 4.2-4.2V4.2c0-2.3-1.8-4.2-4.1-4.2zM175 191.7H25V8.3h150v183.4zm-45-62c0-.1 0-.1 0 0 .4-2 .6-4.1.6-6.2 0-16.1-13.1-29.2-29.2-29.2s-29.2 13.1-29.2 29.2c0 2.1.2 4.1.7 6.1l-.1.1-25 41.7c-.9 1.5-.8 3.4.3 4.8s2.9 1.9 4.6 1.3l22.6-7.5 14.8 14.8c.8.8 1.9 1.2 2.9 1.2.4 0 .7 0 1.1-.2 1.4-.4 2.6-1.5 2.9-3l4.3-17.2 4.3 17.2c.4 1.5 1.5 2.6 2.9 3 .4.1.7.2 1.1.2 1.1 0 2.2-.4 2.9-1.2l14.8-14.8 22.6 7.5c1.7.5 3.5 0 4.6-1.3 1.1-1.4 1.2-3.3.3-4.8L130 129.7zm-28.6-27.1c11.5 0 20.8 9.3 20.8 20.8s-9.3 20.8-20.8 20.8-20.8-9.3-20.8-20.8c0-11.4 9.4-20.8 20.8-20.8zm-10.6 71.1l-11.5-11.5c-1.1-1.1-2.8-1.5-4.3-1L60.8 166l16.1-26.8c4.3 6.7 11.2 11.5 19.3 13l-5.4 21.5zm37-12.5c-1.5-.5-3.1-.1-4.3 1L112 173.7l-5.4-21.5c8.1-1.5 15-6.3 19.3-13L142 166l-14.2-4.8zm-65.3-132c0-2.3 1.9-4.2 4.2-4.2h66.7c2.3 0 4.2 1.9 4.2 4.2s-1.9 4.2-4.2 4.2H66.7c-2.3-.1-4.2-1.9-4.2-4.2zm4.2 54.1c-2.3 0-4.2-1.9-4.2-4.2s1.9-4.2 4.2-4.2h66.7c2.3 0 4.2 1.9 4.2 4.2s-1.9 4.2-4.2 4.2H66.7zM37.5 54.2c0-2.3 1.9-4.2 4.2-4.2h116.7c2.3 0 4.2 1.9 4.2 4.2s-1.9 4.2-4.2 4.2H41.7c-2.3-.1-4.2-1.9-4.2-4.2z"></path>
        </g>
      </svg>
    ),
  },
];

export const ABOUT_US = [
  {
    title: "Mission",
    description:
      "BDCS explores and realizes opportunities with the provision of <strong>high quality</strong> in accounting and audit services that go beyond <strong>cost reductions</strong>. BDCS ensures <strong>time efficiency and reduced overheads</strong>, resulting in our clients ability to have a greater focus on other areas of their business.",
  },
  {
    title: "Vision",
    description:
      "To be recognized as <strong>one of the top outsourcing companies</strong> as a center of excellence for providing accounting and audit services.<br/>We aim to do that with the efforts of our dedicated team through <strong>commitment</strong> to <strong>excellence</strong>, <strong>integrity</strong> and <strong>responsiveness</strong>.",
  },
  {
    title: "Values",
    description:
      "<strong>Quality</strong> – At BDCS we strive for quality that delivers premium value to our clients.<br /><strong>Teamwork</strong> – We work together to meet the needs of our clients and to help BDCS excel.<br /><strong>Respect for people</strong> – We value our people, encourage their development, recognize and reward their performance.<br /><strong>Excellence</strong> – We are dedicated to our work to retain our image for providing excellent and high quality service.<br /><strong>Integrity</strong> – Integrity builds trust. We are committed to individual and organizational success through open and honest cooperation.",
  },
];

export const OUR_TEAM = [
  {
    name: "Dritan Dragović",
    image: dritand,
    role: "Co-Founder",
    bio: `Dritan Dragović (residing in the Netherlands) is a co-founder of BD Corporate Services with previous
          experience of more than 10 years with Big 4 firms, KPMG Netherlands and EY Montenegro. Dritan
          provided assurance services to wide range of international and local Dutch clients in banking sector,
          real estate and asset management. Dritan has experience in audits reporting under both Dutch
          GAAP and IFRS.`,
    linkedin: `https://www.linkedin.com/in/dritan-dragovic/`,
    phone: "",
    email: "dritan.dragovic@bdcs.me",
  },
  {
    name: "Boban Čabarkapa",
    image: bobanc,
    role: "Co-Founder & CEO",
    bio: `As a co-founder and CEO of BD Corporate Services (BDCS) Boban is experienced finance professional with more than 12 years of experience in auditing. 
          He started his career at EY Montenegro in 2011 and before joining KC Audit he worked at Deloitte Montenegro in the role of audit executive director. 
          In between Boban also worked in EY Stockholm for a period of one year at various companies which gives him a better insight in the challenges and focus areas of companies. 
          He is ACCA member since 2018. Boban’s skill set includes strong practices of International Financial Reporting Standards (IFRS) and International Standards on Auditing (ISA) and Dutch GAAP. 
          Besides, he was part of ISQM1 team for Deloitte Central Europe.`,
    linkedin: `https://www.linkedin.com/in/boban-cabarkapa-acca-674b7388/`,
    phone: "",
    email: "boban.cabarkapa@bdcs.me",
  },
  {
    name: "Eldar Berkovac",
    image: eldarb,
    role: "Partner and CEO of Sarajevo Office",
    bio: `Eldar Berkovac holds a bachelor’s degree in Financial
          Management and a master’s degree in Management
          and Organization from the University of Sarajevo. He has over 13 years of professional experience, including
          7.5 years with EY and 5.5 years in senior finance roles,
          where he managed accounting, payroll, and compliance
          functions across multiple entities. Eldar is an ACCA-
          certified professional with extensive expertise in financial
          planning and regulatory compliance. His leadership
          experience and technical proficiency enable him to
          manage complex projects and deliver strategic insights.`,
    linkedin: `https://www.linkedin.com/in/eldar-berkovac-acca-44b76763/`,
    phone: "",
    email: "eldar.berkovac@bdcs.ba",
  },
  {
    name: "Nenad Vucinic",
    image: nenadv,
    role: "Partner",
    bio: `Nenad Vučinić holds a bachelor’s degree in Economics
          from the Faculty of Economics in Belgrade and is a
          certified ACCA member. He has over 12 years of
          professional experience from EY, auditing and
          consulting, with expertise in financial services and multiple industry sectors. Nenad has provided support in
          implementing accounting standards and delivering
          training programs, demonstrating strong technical
          knowledge and leadership capabilities. His academic
          background and extensive experience position him as a
          highly skilled professional in auditing and financial
          advisory.`,
    linkedin: ``,
    phone: "",
    email: "nenad.vucinic@bdcs.me",
  },
  {
    name: "Milka Jovanoviċ",
    image: milkaj,
    role: "Chief of Legal and HR department",
    bio: `Milka Jovanović brings extensive experience in legal and
          HR operations, supported by over a decade of work in
          corporate governance and compliance. She holds a
          degree in Legal Science from the University of
          Montenegro and has developed strong expertise in
          organizational development and regulatory compliance.
          Milka’s professional background includes leadership
          roles in managing internal processes and supporting
          strategic initiatives. Her commitment to ethical standards
          and organizational excellence makes her a valuable
          contributor to corporate operations.`,
    linkedin: ``,
    phone: "",
    email: "milka.jovanovic@bdcs.me",
  },
  {
    name: "Nikola Radunović",
    image: nikolar,
    role: "Manager",
    bio: `Nikola Radunović is a dedicated professional with a diverse background in audit and accounting.
          With 4 years of experience in external audit, 1 year in accounting, and 3 years in internal audit, Nikola has honed his expertise through roles at Deloitte and other companies in Montenegro.
          His work involves auditing statutory and consolidated financial statements for a variety of companies across sectors such as trade, hospitality, transportation, production and insurance, all in compliance with IFRS standards and local accounting regulations.
          During his tenure in internal auditing, Nikola acquired substantial expertise in risk management, internal controls and corporate governance, particularly within an insurance company and an investment fund.
          Nikola holds a European Master in Official Statistics and is also an ACCA student.`,
    linkedin: `https://www.linkedin.com/in/nikola-radunovi%C4%87-31150730a/`,
    phone: "",
    email: "nikola.radunovic@bdcs.me",
  },
  {
    name: "Drazen Milić",
    image: drazenm,
    role: "Manager",
    bio: `Dražen Milić is an experienced audit professional with
          over eight years of expertise in EY, auditing across
          diverse industries, including banking, real estate, oil and
          gas, and telecommunications. He holds a strong
          academic foundation in economics and auditing,
          complemented by international experience gained during
          a one-year assignment with EY Prague. Dražen has a
          proven track record in managing large-scale
          engagements and leading multidisciplinary teams while
          ensuring compliance with regulatory standards. His
          extensive experience and leadership capabilities make
          him a highly skilled professional in the field of auditing.`,
    linkedin: ``,
    phone: "",
    email: "drazen.milic@bdcs.me",
  },
  {
    name: "Petar Milaš",
    image: petarm,
    role: "Manager",
    bio: `Petar Milaš is an ACCA Member with previous experience of more than 13 years with Big 4 firms, EY
          Serbia and EY Montenegro. In September 2023, Petar joined BDCS support team of Ruitenburg Audit
          B.V. (Netherlands) as an Engagement Manager. Petar provided assurance services to wide range of
          international and local companies in various industry sectors such as clients in Oil&Gas, Insurance,
          Telecommunication, Customer products and services, Technology, Power and utilities, Professional
          services, Pharmaceuticals, Media and entertainment, Real estate, Mining, Diversified industrial
          products manufacturing. Petar has experience in audits reporting under IFRS, US GAAP, German
          GAAP, Luxemburg GAAP and Belgium GAAP. Petar also has experience in performing buy-side due
          diligence of insurance and telecommunication companies.`,
    linkedin: `https://www.linkedin.com/in/petar-milas-acca-55053a6/`,
    phone: "",
    email: "petar.milas@bdcs.me",
  },
  {
    name: "Nikola Marić",
    image: nikolam,
    role: "Assistant Manager",
    bio: `Nikola Marić holds a bachelor’s degree in Economics
          and has over four years of experience in external
          auditing – Deloitte, complemented by two years in
          accounting. His professional background includes audits
          of financial statements across diverse industries such as
          insurance, IT, hospitality, and manufacturing. Nikola’s
          academic achievements and practical experience have
          equipped him with strong technical knowledge and
          analytical skills. He is committed to delivering high-
          quality assurance services and supporting clients in
          meeting regulatory requirements.`,
    linkedin: ``,
    phone: "",
    email: "nikola.maric@bdcs.me",
  },
  {
    name: "Dijana Opančina",
    image: dijanao,
    role: "Assistant Manager",
    bio: `Dijana Opančina is of December 2023 an Audit Assistant Manager of BDCS support team for the Dutch market with previous experience at Deloitte Montenegro.
          Before joining BDCS, she was part of Deloitte Montenegro for six years where she has acquired experience in performing audit of consolidated and statutory financial statements prepared in accordance with IFRS and local accounting standards.`,
    linkedin: `https://www.linkedin.com/in/dijana-drobnjak-3b4693198/`,
    phone: "",
    email: "dijana.opancina@bdcs.me",
  },
  {
    name: "Hasija Isaković",
    image: hasijai,
    role: "Assistant Manager",
    bio: `Hasija Isaković is an experienced finance professional
          with a master’s degree in Economics from the
          International University of Sarajevo. She has ten years
          of experience in finance, including eight years in auditing
          -EY and Finance teams, with exposure to industries
          such as banking, retail, pharmaceuticals, and
          construction. Her academic background and extensive
          professional experience have equipped her with strong
          analytical and leadership skills. Hasija is committed to
          delivering high-quality audit services and ensuring
          compliance with international standards.`,
    linkedin: ``,
    phone: "",
    email: "hasija.isakovic@bdcs.ba",
  },
  {
    name: "Milos Vuletić",
    image: milosv,
    role: "Assistant Manager",
    bio: `Milos Vuletić brings six years of professional experience
          from EY assurance services, supported by a strong
          academic background in economics and finance. He has
          worked extensively on audits and financial reporting
          under International Financial Reporting Standards
          (IFRS) and local frameworks. His expertise spans
          multiple industries, including banking,
          telecommunications, oil and gas, and pharmaceuticals.
          Milos combines technical proficiency with practical
          experience, enabling him to deliver high-quality
          assurance services and valuable insights.`,
    linkedin: ``,
    phone: "",
    email: "milos.vuletic@bdcs.me",
  },
  {
    name: "Jelena Simić",
    image: jelenas,
    role: "Senior",
    bio: `Jelena Simić is currently pursuing a master’s degree in
          Auditing and Controlling at the Faculty of Economics in
          Belgrade, building on her bachelor’s degree in auditing
          from the same institution. She has gained professional
          experience in finance and auditing through roles at
          Deloitte, KPMG, where she developed expertise in audit
          methodologies and financial analysis. Jelena’s academic
          achievements and practical exposure have equipped her
          with strong analytical and organizational skills. She is
          committed to continuous learning and delivering high-
          quality results in auditing and financial management.`,
    linkedin: ``,
    phone: "",
    email: "jelena.simic@bdcs.me",
  },
  {
    name: "Bojana Bošković",
    image: bojanab,
    role: "Senior",
    bio: `Bojana Bošković is of September 2023 an Audit Senior of BDCS support team for Ruitenburg Audit
          B.V. (Netherlands) with previous experience at Deloitte Montenegro. During her engagement at
          Deloitte Montenegro, Bojana audited businesses and insurance companies from three countries
          (Montenegro, Serbia, and North Macedonia). Bojana is an ACCA student. Also, Bojana is experienced
          in project management in the NGO sector.`,
    linkedin: `https://www.linkedin.com/in/bojana-bo%C5%A1kovi%C4%87-b5029222b/`,
    phone: "",
    email: "bojana.boskovic@bdcs.me",
  },
  {
    name: "Bojana Vučićević",
    image: bojanav,
    role: "Senior",
    bio: `Bojana Vucicevic is an Audit Senior with over four years
          of professional experience. She began her career in
          accounting and transitioned into audit, specializing in
          IFRS and Dutch GAAP. Bojana holds a Bachelor&#39;s
          degree in Accounting and Auditing and a Master&#39;s
          degree in Financial Controlling from the Faculty of
          Economics in Belgrade. She has also completed the F-
          level of the ACCA certification and is currently pursuing
          full ACCA qualification.
          Her audit experience spans investment funds and fintech
          companies, as well as manufacturing and fashion
          industries. Bojana brings valuable international exposure
          from her time working at BDO Germany and completing
          her Master&#39;s studies at HWR Berlin.`,
    linkedin: ``,
    phone: "",
    email: "bojana.vucicevic@bdcs.me",
  },
  {
    name: "Matija Vujović",
    image: matijav,
    role: "Senior",
    bio: `Matija Vujović has over four years of professional
          experience in auditing from EY. His expertise spans
          audits of annual financial statements across industries
          such as banking, telecommunications, hospitality, and
          manufacturing. Matija’s academic background in
          economics complements his practical experience,
          enabling him to apply rigorous methodologies and
          ensure compliance with regulatory standards. His
          diverse experience positions him as a highly skilled and
          adaptable professional in the field of auditing.`,
    linkedin: ``,
    phone: "",
    email: "matija.vujovic@bdcs.me",
  },
  {
    name: "Ivan Vušković",
    image: ivanv,
    role: "Senior",
    bio: `Ivan Vuskovic holds a bachelor’s degree in Economics
          from the University of Belgrade and has 3.5 years of
          professional experience from EY. His experience spans
          multiple industries, including banking, oil and gas,
          consumer products, and retail, providing him with a
          comprehensive understanding of diverse regulatory
          frameworks. Ivan’s academic background and practical
          exposure have equipped him with strong analytical and
          problem-solving skills. He is committed to delivering
          high-quality audit services and supporting clients in
          achieving compliance and operational excellence.`,
    linkedin: ``,
    phone: "",
    email: "ivan.vuskovic@bdcs.me",
  },
  {
    name: "Emina Konak",
    image: eminak,
    role: "Senior",
    bio: `Emina Konak holds a master degree in Management
          – Accounting and Auditing from the University of
          Sarajevo and is completing her master’s studies in the
          same field. She has professional experience in auditing
          from EY, where she gained exposure to clients across
          banking, telecommunications, retail, and manufacturing.
          Emina’s academic and professional background has
          equipped her with strong analytical and organizational skills. She is committed to advancing her expertise in
          financial auditing and reporting.`,
    linkedin: ``,
    phone: "",
    email: "emina.konak@bdcs.ba",
  },
  {
    name: "Merisa Aganbegović",
    image: merisaa,
    role: "Senior",
    bio: `Merisa Aganbegović holds a bachelor’s degree in
          Economics from the University of Banja Luka and brings
          eight years of professional experience in finance,
          including four years in EY Bosnia and Hercegovina and
          four years in accounting. She has worked on audits
          across various industries, gaining expertise in financial
          reporting and compliance. Merisa’s academic foundation
          and practical experience enable her to approach
          complex financial tasks with precision and efficiency.
          She is dedicated to continuous professional
          development and excellence in auditing.`,
    linkedin: ``,
    phone: "",
    email: "merisa.aganbegovic@bdcs.ba",
  },
  {
    name: "Zerina Sikira",
    image: zerinas,
    role: "Senior",
    bio: `Zerina Sikira is currently pursuing a master’s degree in
          Marketing Management at the Faculty of Economics in
          Sarajevo. She hold a bachelor’s degree in Financial
          Management at the same faculty at University of
          Sarajevo. She has gained professional experience in
          accounting and auditing through roles at EY Sarajevo,
          where she worked for over two years. She demonstrates
          a strong commitment to continuous learning and
          consistently delivers high-quality outcomes in auditing
          and financial management.`,
    linkedin: ``,
    phone: "",
    email: "zerina.sekira@bdcs.ba",
  },
  {
    name: "Anđelina Rakčević",
    image: andjelinalar,
    role: "Experience Assistant",
    bio: `Anđelina Rakčević holds a degree from the Faculty of
          Economics and is currently advancing her education
          through a master’s program in Finance and Accounting.
          She has developed strong analytical and problem-
          solving skills through her work in auditing,
          complemented by a brief internship at EY that introduced
          her to global best practices. Her combination of
          academic knowledge and hands-on experience provides
          a solid foundation in auditing and financial reporting.
          Anđelina is passionate about professional growth and
          consistently aims to deliver accurate and high-quality
          results.`,
    linkedin: ``,
    phone: "",
    email: "andjelina.rakcevic@bdcs.me",
  },
  {
    name: "Jovana Janketić",
    image: jovanaj,
    role: "Experience Assistant",
    bio: `Jovana Janketić holds a Bachelor’s degree in
          Economics from the Faculty of International Economics,
          Finance and Business at UDG, Montenegro. She has
          strong knowledge in accounting, auditing, economics,
          and finance. For the past two years, Jovana has been
          working on audit engagements for the Dutch market,
          gaining practical experience in statutory audits, financial
          reporting, and compliance. Her work includes supporting
          international clients and contributing to the successful
          execution of complex audit processes.`,
    linkedin: `https://www.linkedin.com/in/jovana-janketi%C4%87-a091aa2a6/`,
    phone: "",
    email: "jovana.janketic@bdcs.me",
  },
  {
    name: "Kristina Radović",
    image: kristinar,
    role: "Experience Assistant",
    bio: `Kristina Radović has professional auditing experience,
          complemented by prior experience in accounting. Her
          background includes work in an accounting agency,
          providing her with a solid foundation in financial
          processes. Kristina’s combined expertise in auditing and
          accounting enables her to approach complex financial
          tasks with precision and efficiency. She is committed to
          continuous professional growth and delivering high-
          quality results in the field of finance.`,
    linkedin: ``,
    phone: "",
    email: "kristina.radovic@bdcs.me",
  },
  {
    name: "Anja Jovanović",
    image: anjaj,
    role: "Assistant",
    bio: `Anja Jovanović is a student at the Faculty of Economics,
          University of Montenegro, focusing her academic studies
          on accounting and auditing. She has gained practical
          experience through engagement in audit work, actively
          participating in audit engagements, and preparing
          documentation and working papers. Her academic
          background and hands-on experience have
          strengthened her analytical skills and attention to detail.
          Anja is highly motivated to continue learning and
          developing her expertise in auditing and financial
          reporting.`,
    linkedin: ``,
    phone: "",
    email: "anja.jovanovic@bdcs.me",
  },
  {
    name: "Bojan Živković",
    image: bojanz,
    role: "Assistant",
    bio: `Bojan Zivković holds a bachelor’s degree in Finance and
          Accounting from the University of Montenegro and is
          currently pursuing a master’s degree in the same field.
          He has gained practical experience in auditing, where he
          developed proficiency in performing substantive
          procedures and supporting key steps in audit
          engagements. Bojan’s academic background and
          professional exposure have equipped him with strong
          analytical and problem-solving skills. He is committed to
          continuous learning and delivering high-quality
          assurance services.`,
    linkedin: ``,
    phone: "",
    email: "bojan.zivkovic@bdcs.me",
  },
  {
    name: "Emina Aslama",
    image: eminaa,
    role: "Assistant",
    bio: `Emina Aslama holds a master’s degree in Economics
          from the University of Sarajevo and has over seven
          years of experience in finance, accounting, and
          compliance. Her professional background includes roles
          in financial reporting, payroll, audits, and taxation, where
          she developed expertise in managing month-end
          processes and ensuring regulatory compliance. Emina’s
          academic achievements and practical experience make
          her a highly skilled professional in financial
          management. She is dedicated to maintaining high
          standards of accuracy and efficiency in all aspects of her
          work.`,
    linkedin: ``,
    phone: "",
    email: "emina.aslama@bdcs.ba",
  },
  {
    name: "Milica Jarić",
    image: milicaj,
    role: "Assistant",
    bio: `Milica Jarić holds a bachelor’s degree in Economics and
          has experience in external auditing, complemented by a
          strong IT background. She has completed training in
          Cisco networking, Python programming, and data
          analysis tools such as Power BI. Milica’s combination of
          financial knowledge and technical expertise enables her
          to contribute effectively to assurance services and data-
          driven decision-making. She is dedicated to continuous
          learning and professional growth in both finance and
          technology.`,
    linkedin: ``,
    phone: "",
    email: "milica.jaric@bdcs.me",
  },
  {
    name: "Nina Janković",
    image: nikolinaj,
    role: "Assistant",
    bio: `Nina Janković is a graduate of the Faculty of Economics
          and is currently pursuing a master’s degree in Finance
          and Accounting. She has gained practical experience as
          an auditor, where she developed strong analytical and financial skills. Nina’s academic achievements and
          professional exposure have equipped her with a solid
          understanding of auditing and financial reporting. She is
          dedicated to continuous learning and strives to deliver
          high-quality results in every engagement.`,
    linkedin: ``,
    phone: "",
    email: "nina.jankovic@bdcs.me",
  },
  {
    name: "Nikolina Koljević",
    image: nikolinak,
    role: "Assistant",
    bio: `Nikolina Koljević earned a bachelor’s degree in
          Economics from the University of Montenegro and is
          currently pursuing a master’s degree in Quantitative
          Economics. She has professional experience in
          accounting, where she developed strong analytical and
          organizational skills. Her academic focus on quantitative
          methods, combined with practical exposure to financial
          reporting, positions her as a detail-oriented and
          technically proficient professional. Nikolina is dedicated
          to advancing her knowledge in economics and finance
          through continuous learning.`,
    linkedin: ``,
    phone: "",
    email: "nikolina.koljevic@bdcs.me",
  },
  {
    name: "Stefan Pavlović",
    image: stefanp,
    role: "Assistant",
    bio: `Stefan Pavlović holds a bachelor’s degree in Finance
          and Accounting from the Faculty of Economics,
          University of Montenegro, and is currently completing his
          master’s studies in the same field. His academic
          background has provided him with a solid understanding
          of financial analysis, accounting principles, and auditing
          methodologies. Stefan has complemented his education
          with practical experience in auditing, applying theoretical
          knowledge to real-world financial processes. He
          continues to develop his expertise through ongoing
          education and professional engagement.`,
    linkedin: ``,
    phone: "",
    email: "stefan.pavlovic@bdcs.me",
  },
  {
    name: "Anastasiia Ishkinina",
    image: anastasiiai,
    role: "Junior",
    bio: `Anastasiia Ishkinina holds a bachelor’s degree in
          International Economics, Finance, and Business from
          the University of Donja Gorica and is currently pursuing
          a master’s degree in Official European Statistics
          (EMOS). Her academic background provides a strong
          foundation in finance, statistics, and management, while
          her specialization focuses on applied statistics and
          economic analysis. Anastasiia has gained practical
          experience in auditing, contributing to audit procedures
          and documentation. She is committed to building a professional career in audit through continuous learning
          and hands-on experience.`,
    linkedin: ``,
    phone: "",
    email: "anastasiia.ishkinina@bdcs.me",
  },
  {
    name: "Emina Čobović",
    image: eminac,
    role: "Junior",
    bio: `Emina Čobović is a student at the Faculty of Economics,
          University of Montenegro, with a focus on finance,
          auditing, and accounting. She enhanced her academic
          knowledge by attending the School of Audit organized by
          her faculty and leading audit firms, gaining practical
          insights into audit methodologies. Emina has
          participated in various academic programs that
          strengthened her understanding of financial reporting
          and compliance. Her dedication to learning and professional growth reflects her ambition to build a
          successful career in auditing.`,
    linkedin: ``,
    phone: "",
    email: "emina.cobovic@bdcs.me",
  },
  {
    name: "Iva Milačić",
    image: ivam,
    role: "Junior",
    bio: `Iva Milačić is currently pursuing a master’s degree in
          Official European Statistics through the EMOS program,
          building on her strong academic foundation in
          economics and finance. She has accumulated practical
          experience in accounting and financial operations, which
          has strengthened her technical knowledge and attention
          to detail. Her academic focus on applied statistics and
          economic analysis complements her professional
          experience, enabling her to approach complex financial
          tasks with precision. Iva is dedicated to expanding her
          expertise in data-driven financial decision-making.`,
    linkedin: ``,
    phone: "",
    email: "iva.milacic@bdcs.me",
  },
  {
    name: "Ivana Zvicer",
    image: ivanaz,
    role: "Junior",
    bio: `Ivana Zvicer is currently pursuing a bachelor’s degree at
          the Faculty of Economics, University of Montenegro (UCG),
          where she has developed a strong foundation in
          accounting, auditing, economics, and finance. Her
          academic journey has equipped her with analytical and
          problem-solving skills essential for financial and audit
          processes. Ivana has gained practical experience in
          supporting audit and accounting tasks, demonstrating
          attention to detail and adaptability. She is committed to
          continuous learning and professional development in the
          field of finance and auditing.`,
    linkedin: ``,
    phone: "",
    email: "ivana.zvicer@bdcs.me",
  },
  {
    name: "Jovana Popović",
    image: jovanap,
    role: "Junior",
    bio: `Jovana Popović holds a bachelor’s degree from the
          Faculty of Economics in Podgorica and is currently
          pursuing a master’s degree in Finance and Accounting.
          Her academic experience includes two Erasmus+
          exchanges in Warsaw and Timișoara, which broadened
          her international perspective and adaptability. Jovana
          actively engaged in student leadership roles, serving as
          Vice President of SRDSEM “Ekomen” and as a member
          of the Council of the Best Students, where she
          developed strong organizational and teamwork skills.
          She combines academic excellence with practical
          experience, positioning herself as a promising
          professional in finance and auditing.`,
    linkedin: ``,
    phone: "",
    email: "jovana.popovic@bdcs.me",
  },
  {
    name: "Milena Raičević",
    image: milenar,
    role: "Junior",
    bio: `Milena Raičević completed her bachelor’s degree at the
          Faculty of Economics in Podgorica and is currently
          pursuing a master’s degree in Business Informatics. She
          has gained significant professional experience as a
          Project Assistant at NLB Bank, where she developed
          strong communication, organizational, and problem-
          solving skills. Her academic background, combined with
          practical exposure to financial and project management
          processes, has equipped her with adaptability and analytical capabilities. Milena is committed to continuous
          learning and advancing her expertise in finance and
          audit.`,
    linkedin: ``,
    phone: "",
    email: "milena.raicevic@bdcs.me",
  },
  {
    name: "Nemanja Kljajević",
    image: nemanjak,
    role: "Junior",
    bio: `Nemanja Kljajević holds a bachelor’s degree from the
          Faculty of Economics, University of Montenegro. His
          professional background includes experience as a
          Pricing and Revenue Analyst and as a Project Assistant
          in the banking sector, where he developed strong
          analytical and organizational skills. Nemanja has also
          participated in international cultural exchange programs,
          which enhanced his adaptability and cross-cultural
          communication abilities. His academic and professional
          experiences provide a solid foundation for continued
          growth in finance and auditing.`,
    linkedin: ``,
    phone: "",
    email: "nemanja.kljajevic@bdcs.me",
  },
  {
    name: "Sabrina Luković",
    image: sabrinal,
    role: "Junior",
    bio: `Sabrina Luković is currently pursuing a degree in
          Finance at the University of Donja Gorica, supported by
          a strong academic foundation in economics and English.
          She has gained practical experience in accounts
          payable and finance operations through work with
          international companies, which strengthened her
          understanding of financial processes. Sabrina combines
          academic knowledge with hands-on experience,
          demonstrating adaptability and attention to detail. She is
          eager to continue developing her skills in auditing and
          financial management.`,
    linkedin: ``,
    phone: "",
    email: "sabrina.lukovic@bdcs.me",
  },
  {
    name: "Ivana Jovanović",
    image: ivanaj,
    role: "Business Support Manager",
    bio: `Ivana Jovanović brings extensive experience in
          customer relations and insurance representation,
          supported by more than seventeen years of professional
          work in the insurance sector. She holds a degree in
          Political Science from the University of Montenegro,
          where she developed a strong foundation in public administration, communication, and organizational
          processes.
          Throughout her career, Ivana has built solid expertise in
          insurance services, client support, and operational
          coordination. Known for her dedication, responsibility,
          and professionalism, she consistently upholds high
          standards in her work.`,
    linkedin: ``,
    phone: "",
    email: "ivana.jovanovic@bdcs.me",
  },
];
